...
public class Main {
    ...
    private static void validateConfig() {
        // si se suministra DB_URL o DATABASE_URL, no es necesario verificar cada campo
        String jdbc = Config.getJdbcUrl();
        System.out.println("Config JDBC URL=" + jdbc);
        if (jdbc == null || jdbc.isBlank()) {
            ...
            for (String k : keys) {
                ...
                if (v == null || v.isBlank()) {
                    System.err.println("Falta variable de configuración obligatoria: " + k + " en sim.properties, .env o variables de entorno");
                    throw new IllegalStateException("Variable faltante: " + k);
                }
            }
        }
        // credenciales siempre necesarias
        String user = Config.get("DB_USER");
        String pass = Config.get("DB_PASSWORD");
        if (user == null || user.isBlank() || pass == null || pass.isBlank()) {
            throw new IllegalStateException("DB_USER y DB_PASSWORD deben estar definidos");
        }
    }
